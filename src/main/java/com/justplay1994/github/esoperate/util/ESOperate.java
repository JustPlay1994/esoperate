...
public class ESOperate {
    ...
    public static String getIndexName(String dbName, String tbName){
        return tbName.toLowerCase()+"@"+dbName;
    }
    public static final String esURL = "http://10.190.55.58:9200/";
...
//        return linkedHashMaps;
    ...
    public static List<LinkedHashMap> extractQueryResult(HashMap queryResult){
        ...
        for(int i = 0; i < list.size(); ++i){
            ...
            String id = ((LinkedHashMap)list.get(i)).get("_id").toString();
            /*如果在表里面没有id字段，则把es的id字段加进去*/
            if (map1.get("ID")==null && map1.get("id")==null)
                map1.put("ID",id);
            resultList.add(map1);
        }
        return resultList;
